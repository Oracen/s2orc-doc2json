...
def normalize(path, out_dir, write_logs=True):
    ...
    # give up
    if main_tex_path is None:
        log(("couldn't find main tex file in dump archive {}" "").format(fn))
    ...
    with tempfile.TemporaryDirectory() as tmp_dir_path:
        ...
        if bbl_files:
            ...
            latexpand_args = ["latexpand", main_tex_path, "--output", temp_tex_fn]
...
def latex_to_xml(
    tex_file: str, out_dir: str, out_file: str, err_file: str, log_file: str
):
    """
    ...
    :return:
    """
    with open(os.devnull, "w") as devnull, open(err_file, "a+") as err_f, open(
        log_file, "a+"
    ) as skip_f:
        # run tralics
        tralics_args = [
